...
void LibcameraApp::StartCamera() {
    ...
    if (options_->verbose) {
        ...
        "\n options_->roi_height: " << options_->roi_height <<
        std::endl;
    }
    ...
    if (!controls_.contains(controls::ScalerCrop) && (options_->roi_width != 0 || options_->roi_height != 0)) {
        ...
        int x = options_->roi_x;
        int y = options_->roi_y;
        int w = options_->roi_width;
        int h = options_->roi_height;
...
void
LibcameraApp::StreamDimensions(Stream const *stream, unsigned int *w, unsigned int *h, unsigned int *stride) const {
