...
void MjpegEncoder::encodeThread(int num)
{
  ...
  size_t exif_buffer_lens[NUM_FRAMES];
  ...
  while (true) {
    ...
    index = frames % NUM_FRAMES;
    {
      ...
      if(doExif_)
      {
          CreateExifData(encode_item, exif_buffer[index], exif_buffer_lens[index]);
      }
      if(doPrimsample_)
      {
